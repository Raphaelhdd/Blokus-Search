...
class Board:
    ...
    def __init__(self, board_w, board_h, num_players, piece_list, starting_point=(0, 0)):
        self.board_w = board_w # 7
        self.board_h = board_h # 4
        self.num_players = num_players # 1
        self.scores = [0] * self.num_players # [0]

        self.state = np.full((board_h, board_w), -1, np.int8) #4 ligne de 7 colonnes -1
        ...
        for row in range(self.board_h):
            for col in range(self.board_w):
                if self.state[row, col] == -1:
                    ...
                    out_str.append(str(self.state[row, col]))
            out_str.append('\n')
        return ''.join(out_str)
